...
def fetch_llm_completion(
    ...
    *,
    ...
    sys_default = (
        ...
        "Не используй «в нашей компании», «мы нанимаем». Отвечай по-русски: 3–7 предложений, конкретика. Без HTML."
    )
    body = {
        "model": model,
        "messages": [
...
_CAREER_CHAT_SYSTEM = """Ты карьерный консультант для молодёжи 14–30 в России.

Ты не из компании пользователя и не работодатель. Запрещено писать «в нашей компании», «у нас нанимаем».

Обязательно:
- Только русский язык (кроме имён брендов при необходимости).
- Отвечай на последнее сообщение пользователя; не выдумывай лишний small talk.
- Опирайся на блоки «Контекст разбора» и «Направления» (A, B, C), если они переданы.
- 6–12 предложений, связный текст. Без HTML."""
...
def _mock_career_chat_reply(last_user: str) -> str:
    ...
    return (
        ...
        f"По теме «{clip}» — уточните город, формат работы и что важнее: обучение, деньги или баланс."
    )
...
def chat_completion(
    ...
    text, _, _ = career_coach_chat_reply(messages, context_summary, directions_hint)
    return text
